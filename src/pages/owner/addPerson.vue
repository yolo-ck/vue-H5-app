  ...
  <div class="page">
    ...
    <div class="scroll-list-wrap">
      <cube-scroll ref="scroll">
        ...
        <address-list :first="falg" ref="addressComp" :data="resubmitAddress" :sign="true"/>
        <cube-form
...
export default {
  ...
  data() {
    return {
      ...
      addressStatus: "",
      code: 1,
      regIdCard: /(^[1-9]\d{5}(18|19|([23]\d))\d{2}((0[1-9])|(10|11|12))(([0-2][1-9])|10|20|30|31)\d{3}[0-9Xx]$)|(^[1-9]\d{5}\d{2}((0[1-9])|(10|11|12))(([0-2][1-9])|10|20|30|31)\d{2}[0-9Xx]$)/
    };
  },
  ...
  watch: {
    ...
    addModel: {
      ...
      handler(val) {
        if (!this.isResubmit) {
          // debugger
          if (this.regIdCard.test(val.Idnumber)) {
            this.$http
              .get(
                HOST +
                  "/a/user/audit/getUserRegistrationByIdNo?idNo=" +
                  val.Idnumber
              )
              .then(response => {
                ...
                if (!response.data.display) {
                  if (response.data.msg) {
                    Dialog.alert({
                      ...
                      // this.addModel.Idnumber = "";
                       this.addModel.nameValue = "";
                      this.addModel.Idnumber = "";
                      this.addModel.sexValue = "";
                      this.addModel.nationValue = "";
                      this.addModel.areaValue = "";
                    });
                  }
                } else {
                  if (response.data.msg && !response.data.isDelete) {
                    Dialog.confirm({
                      message: response.data.msg
                    })
                      .then(() => {

                        this.again(response.data.data);
  ...
  methods: {
    ...
    submitHandler(e) {
      ...
      if (this.filtedAddresses.length > 1) {
        Toast("新添加人员只能绑定一个地址！");
        return;
      }
      ...
      // that.regIdCard = /(^[1-9]\d{5}(18|19|([23]\d))\d{2}((0[1-9])|(10|11|12))(([0-2][1-9])|10|20|30|31)\d{3}[0-9Xx]$)|(^[1-9]\d{5}\d{2}((0[1-9])|(10|11|12))(([0-2][1-9])|10|20|30|31)\d{2}[0-9Xx]$)/;
      if (!reg.test(this.addModel.phoneValue)) {
        Toast("手机号码有误，请重新输入");
        return;
      }
      if (!that.regIdCard.test(this.addModel.Idnumber)) {
        Toast("身份证号码有误，请重新输入");
        return;
      }
