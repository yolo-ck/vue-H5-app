    ...
    <div class="scroll-list-wrap">
      <cube-scroll ref="scroll" :options="options">
        ...
        <div id="owner" v-if="roleValue === '业主'">
          ...
          <address-list first="true" :firstBind="roleLenght" ref="addressComp"/>
          <cube-form
...
<script>
 ...
 beforeRouteEnter(to, from, next) {
    next(vm => {
      ...
      if (from.path.indexOf("selectRoom1") > -1 || from.path.indexOf("selectRoom") > -1) {
        vm.$store.commit("SET_KEEP_ALIVE", ["firstBinding"]);
      }
    });
  },
  beforeRouteLeave(to, from, next) {
    ...
    } else if (to.path.indexOf("login") > -1||to.path.indexOf("home") > -1) {
...

   {
    ...
    methods: {
      ...
      addShop() {
        ...
        //    this.$refs.firstAddShop.addItem(filtedAddresses[0]);
      ...
      submitHandler(e) {
        ...
        // this.papersPhoto="ceshi"
        // this.hochFormat="ceshi"

        //    if(this.roleLenght){
        //   if(filtedAddresses.length>50){
        //     Toast("业主初次绑定地址不得超过50个");
        //     return;
        //   }
          
        // }else{
        //      if(filtedAddresses.length>3){
        //     Toast("非业主初次绑定地址不得超过3个");
        //     return;
        //   }
        // }
        if(filtedAddresses.length>1){
            Toast("初次绑定地址不得超过1个");
            return;
        }
        ...
        if (this.roleValue == "业主") {
          ...
          this.sendeducationValue = this.ownerModel.educationValue;
           ...
           this.sendpoliticsStatusValue = this.ownerModel.politicsStatus;
            this.sendmarryValue = this.ownerModel.marry;
             this.sendhomeTpyeValue = this.ownerModel.homeTpye;
              this.sendliveForValue = this.ownerModel.liveFor;
        ...
        if (this.roleValue == "新居民") {
          ...
          this.sendjobValue = this.stayModel.jobValue;
          this.sendphoneValue = this.stayModel.phoneValue;
          // this.roleValue = 1;
          this.sendeducationValue = this.stayModel.educationValue


                this.sendpoliticsStatusValue = this.stayModel.politicsStatus;
            this.sendmarryValue = this.stayModel.marry;
             this.sendhomeTpyeValue = this.stayModel.homeTpye;
              this.sendliveForValue = this.stayModel.liveFor;
      ...
      this.sendpoliticsStatusValue = this.residentModel.politicsStatus;
            this.sendmarryValue = this.residentModel.marry;
             this.sendhomeTpyeValue = this.residentModel.homeTpye;
              this.sendliveForValue = this.residentModel.liveFor;
      ...
      toast1.show();
      var userType
      if(that.roleValue=="业主"){
          userType=0
      }else if(that.roleValue=="新居民"){
          userType=1
      }else if(that.roleValue=="常住用户"){
          userType=2
      }
        ...
        this.$http({
          ...
          data: {
            ...
            plitical: that.sendpoliticsStatusValue,//政治面貌
            marital: that.sendmarryValue,//婚姻状况
            residenceType: that.sendhomeTpyeValue,//住所类别
            liveReason: that.sendliveForValue,//居住事由
