...
export default {
  name: "AddressList",
  props: ["first", "data", "addshop", "firstAddShop", "firstBind"],
  data() {
    return {
      addresses: [],
  ...
  methods: {
    ...
    addRow() {
      if (this.userType == "firstBinding") {
        this.auditSize = this.$store.getters.getUserInfo.total - this.initSize;

        if (this.first) {
          this.auditSize = 0;
        }
        // if (this.data) {
        if (this.addresses.length >= 1 - this.auditSize) {
          Toast({
            message: "初次绑定每人最多只能提交绑定1个地址！",
            position: "bottom",
            duration: 3000
          });
        ...
        // if (this.$store.state.staffId) {
        //   const urls = HOST + "/a/mobile/user/register/getUserInfo";
        //   this.$http({
        //     method: "post",
        //     url: urls,
        //     data: {
        //       userId: this.$store.state.staffId
        //     }
        //   }).then(result => {
        //     var total = result.data.total;
        //     console.log(total);
        //     this.auditSize = total - this.initSize;
        //   });
        // } else {
        this.auditSize = this.$store.getters.getUserInfo.total - this.initSize;
        // }
        ...
        if (this.data) {
          if (this.addresses.length >= ADDRESS_COUNT - this.auditSize) {
            Toast({
              message: "每人最多只能提交绑定50个地址！(包含正在审核的)",
              position: "bottom",
              duration: 3000
            });
          ...
          if (this.addresses.length >= MAX_ADDRESS_COUNT - this.auditSize) {
            Toast({
